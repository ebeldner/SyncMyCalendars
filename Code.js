...
var USER_SETTINGS_KEY = 'BUSYBLOCKER_USER_SETTINGS';

// Map Visibility enum or string to a serialized key.
function visibilityKey(value) {
  if (!value) return null;
  if (typeof value === 'string') return value;
  // Enum values stringify to their key names in Apps Script.
  return String(value);
}

function coerceVisibility(value) {
  var key = visibilityKey(value);
  if (key && CalendarApp.Visibility[key]) {
    return CalendarApp.Visibility[key];
  }
  return UserSettingsDefaults.visibility || CalendarApp.Visibility.DEFAULT;
}
...
function mergeSettings(defaults, overrides) {
  ...
  for (var key in defaults) {
    ...
    if (overrides && overrides.hasOwnProperty(key) && overrides[key] !== undefined && overrides[key] !== null) {
...
function normalizeIncludeDays(days, fallback) {
  var src = Array.isArray(days) ? days : (typeof days === 'string' ? days.split(',') : []);
  var normalized = [];
  for (var i = 0; i < src.length; i++) {
    var n = parseInt(src[i], 10);
    if (!isNaN(n) && n >= 0 && n <= 6 && normalized.indexOf(n) === -1) {
      normalized.push(n);
    }
  }
  return normalized.length ? normalized : (fallback || UserSettingsDefaults.includeDays);
}

function normalizeSourceCalendars(sourceCalendars) {
  if (!Array.isArray(sourceCalendars)) return [];
  var normalized = [];
  for (var i = 0; i < sourceCalendars.length; i++) {
    var sc = sourceCalendars[i] || {};
    if (!sc.id || typeof sc.id !== 'string') continue;
    var id = sc.id;
    var title = sc.title || ('Unavailable (' + id + ')');
...
function normalizeSettings(raw) {
  var defaults = UserSettingsDefaults;
  var merged = mergeSettings(defaults, raw || {});
  merged.windowDays = Math.max(1, Math.min(90, parseInt(merged.windowDays, 10) || defaults.windowDays));
  merged.includeDays = normalizeIncludeDays(merged.includeDays, defaults.includeDays);
  merged.destinationEventTitle = (merged.destinationEventTitle && String(merged.destinationEventTitle).trim()) || defaults.destinationEventTitle;
  merged.color = (merged.color && String(merged.color)) || defaults.color;
  merged.removeReminders = !!merged.removeReminders;
  merged.visibility = coerceVisibility(merged.visibility);
  merged.sourceCalendars = normalizeSourceCalendars(merged.sourceCalendars);
  return merged;
}

function loadStoredUserSettings() {
  var props = PropertiesService.getUserProperties();
  var raw = props.getProperty(USER_SETTINGS_KEY);
  if (!raw) return null;
  try {
    return JSON.parse(raw);
  } catch (e) {
    Logger.log('Could not parse stored user settings; using defaults. ' + e);
    return null;
  }
}
...
function saveUserSettings(updates) {
  ...
  var mergedInput = {
    windowDays: updates.windowDays,
    includeDays: updates.includeDays,
    destinationEventTitle: updates.destinationEventTitle,
    color: updates.color,
    visibility: updates.visibility,
    sourceCalendars: updates.sourceCalendars,
    // Hidden/locked: preserve existing values
    removeReminders: current.removeReminders
  };
  var normalized = normalizeSettings(mergedInput);
  var props = PropertiesService.getUserProperties();
  props.setProperty(USER_SETTINGS_KEY, JSON.stringify({
    windowDays: normalized.windowDays,
    includeDays: normalized.includeDays,
    destinationEventTitle: normalized.destinationEventTitle,
    color: normalized.color,
    removeReminders: normalized.removeReminders,
    visibility: visibilityKey(normalized.visibility),
    sourceCalendars: normalized.sourceCalendars
  }));
  return normalized;
}

function loadUserSettings() {
  var stored = loadStoredUserSettings();
  if (stored) {
    return normalizeSettings(stored);
  }
  if (typeof UserSettingsLocal !== 'undefined') {
    return normalizeSettings(UserSettingsLocal);
  }
  return normalizeSettings(UserSettingsDefaults);
}
...
// Only use HTML blob; ignore legacy promoMessage. Admin decides whether to clear description if empty.
function applyDescription(targetDescription, user, admin) {
  var shouldClear = (admin && typeof admin.CLEAR_DESCRIPTION === 'boolean') ? admin.CLEAR_DESCRIPTION : (user && user.clearDescription);
  if (shouldClear && (!targetDescription || targetDescription.length === 0)) {
    return '';
  }
...
function updateExisting(existingEvent, srcEvent, settings, state, sourceId) {
  ...
  var targetDescription = applyDescription(settings.promoHtml, user, admin);
...
function syncAllCalendars() {
  ...
  if (!user.sourceCalendars || user.sourceCalendars.length === 0) {
    Logger.log('No sourceCalendars configured in user settings.');
    return;
  }
...
// ----------------
// ADMIN: Find / update / purge blockers
// ----------------
...
function updateAllBusyBlocks_UserSettings(requireTitleMatch) {
  ...
  for (var i = 0; i < user.sourceCalendars.length; i++) {
    ...
    for (var j = 0; j < matches.length; j++) {
      ...
      var targetDescription = applyDescription(settings.promoHtml, settings.user, settings.admin);
